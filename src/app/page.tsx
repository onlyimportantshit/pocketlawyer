  ...
  return (
    <main ref={containerRef} className="relative bg-white text-slate-900 selection:bg-blue-100 overflow-x-hidden">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center px-6 overflow-hidden">
        ...
        <motion.div style={{ y: blobY }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-blue-400/10 blur-[120px] rounded-full -z-10" />
      </section>
...
function BentoCard({ title, price, icon, color, items, span }: any) {
  return (
    <motion.div variants={itemVariants} whileHover={{ y: -10, scale: 1.01 }} className={`${span} ${color} p-10 rounded-[3.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden relative group`}>
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((item: any) => (
          <Link key={item.name} href={item.href} className="px-6 py-3 rounded-full bg-black/5 backdrop-blur-sm flex justify-between items-center text-xs font-black uppercase tracking-tighter border border-black/5 hover:bg-blue-600 hover:text-white transition-all cursor-pointer group/btn">
